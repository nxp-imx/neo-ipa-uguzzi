...
/*
 * This boolean is a shared variable between IPA when
 ...
 * - only one server socket can be created using the same port
...
private:
	...
	bool embeddedLinesAvailable_{ false };
...
int IPANxpNeo::initializeUguzzi(Size outputSize)
{
	...
	/*
	 * for ovx3c on MMS provided DTP
	 * \todo try deriving the ID from the sensor model. But even then
	 * some code change will be required as they are user defined
	 * and specific to the DTP.
	 */
	initCfg.channel_config[channel_].sensor_id = 2010;
	initCfg.channel_config[channel_].sensor_mode = 0;
		    ...
		    SensorConfig *sensorConfig)
{
	...
	if ((disableIsolation && disableIsolation[0] != '\0') &&
	    gblIpaInitialized.test_and_set()) {
		/* IPA runs in non-isolated mode */
		/* Restrict initialization to 1 single IPA */
		LOG(NxpNeoUguzziIPA, Debug)
			<< "In non-isolated mode, initialization is restricted "
			<< "to 1 single IPA, hence this IPA init is skipped";
		return 0;
	}

	LOG(NxpNeoUguzziIPA, Info) << "IPANxpNeo "
				   << NEO_IPA_UGUZZI_VERSION;
	LOG(NxpNeoUguzziIPA, Debug) << "Initializing for "
				    << settings.sensorModel
				    << " on hardware revision " << hwRevision
				    << "- Sensor id: " << sensorId;
	...
	std::string dtpDir = utils::dirname(settings.configurationFile) + "/uguzzi";
	std::string dtpBin = dtpDir + "/" + "database_" + settings.sensorModel + ".bin";
...
#ifdef USE_LIVE_CONTROL
	LiveControl &liveCtrl = LiveControl::getInstance();
	uint16_t portSocket = 0;
	int ret = liveCtrl.createSocket(portSocket);
	if (ret)
		LOG(NxpNeoUguzziIPA, Error)
			 ...
			 ControlInfoMap *ipaControls)
{
	...
	uint32_t topLines = camHelper_->attributes()->mdParams.topLines;
	metaDataSize_ = topLines * camInfoDtp_[channel_]->frame1_cfg.bytes_per_line;
				 ...
				 const uint32_t rawBufferId)
{
	...
	embeddedLinesAvailable_ = false;
	if (metaDataSize_ && buffers_.count(rawBufferId)) {
		const uint8_t *metadata =
			buffers_.at(rawBufferId).planes()[0].data();
		Span<const uint8_t> embDataBuff(metadata, metaDataSize_);
		int ret = camHelper_->parseEmbedded(embDataBuff, &controls);
		embeddedLinesAvailable_ = (ret == 0);
	}

	/* give access to raw buffer for live tuning */
	rawBufferId_ = rawBufferId;
				   ...
				   const ControlList &sensorControls)
{
	...
	/**
	 * Embedded lines were not available so use the delay line maintained
	 * by the pipeline handler.
	 */
	if (!embeddedLinesAvailable_) {
		if (!frame)
			LOG(NxpNeoUguzziIPA, Info)
				<< "Embedded Lines not available, delay lines are used instead!";
		controls = ControlList(md::controlIdMap);
